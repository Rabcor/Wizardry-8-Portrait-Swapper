...
class GUI:
    def __init__(self, default_portraits, modded_portraits):
        ...
        self.slider_labels = ["Base Portrait", "Eyes Neutral", "Eyes Closed", "Eyes Lidded", "Eyes Angry", 
                        "Eyes Shocked", "Mouth Neutral", "Mouth Partially Open", "Mouth Fully Open", "Mouth Pain"]
        ...
        self.change_button.grid(row=1, column=1, pady=10, sticky=tk.SE, padx=120)
    ...
    def populate_portrait_listbox(self):
        ...
        for key in self.modded_portraits.keys():
            if key.startswith(("PORTRAITS\\LARGE\\", "PORTRAITS/LARGE/")):
                portrait_names.add(os.path.basename(key)[17:-4])
    ...
    def change_portrait(self, files = None):        
        if not files:
            ...
            title="Select Images",
            filetypes=[("PNG Files", "*.png"), ("PNG Files", "*.PNG")])
            if not files:return
        ...
        for file_path in files:
            try:
                ...
                elif (width == 45 and height == 36) or (width == 46 and height == 36):
                    if img.mode == 'RGBA':
                        img = img.convert('RGB')
                    raw_data = list(img.getdata())
    ...
    def restore_defaults(self):
        ...
        message_label = ttk.Label(dialog, text="Which portraits would you like to restore to default?", 
                                background=self.bg_color, foreground=self.fg_color, font=('SegoeUI', self.font_size + 2), wraplength=340)
