...
class STI8:
    def __init__(self, source: str | bytes | None = None):   
        ...
        if source is not None:
            ...
            try:
                if self.etrle: self.atlas = etrle_decompress(self.atlas,self.palette,self.width, self.height)
    ...
    def _quantize_atlas(self):
        ...
        if color_count > 254: # Quantize
            ...
            try: # To use libimagequant for quantization
                ...
                # Apply custom alpha replacement (if any pixel matches `alpha` RGB and has A=0)
                ...
                # Insert special colors into palette
